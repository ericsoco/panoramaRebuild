...
export default class TextList extends Component {
  ...
  buildItemCache() {
    this.anchors = [];
    this.anchorMap = {};
    const listItems = this.listContainer.querySelectorAll('.text-list--item');
  ...
  getClosestItemNode(elm) {
    for ( ; elm && elm !== document; elm = elm.parentNode ) {
      if ( elm.classList.contains( 'text-list--item' )) {
        return elm;
      }
    }
    return false;
  ...
  onClickHandler(evt) {
    ...
    const node = target.classList.contains( 'text-list--item' ) ? target : this.getClosestItemNode(target);
  ...
  renderItems () {
    ...
    return items.map((item, idx) => {
      ...
      return <TextListItem key={key} renderItem={renderItem} index={idx} identifier={this.coerceKey(key)} dirty={dirty} />;
    });
  }
  ...
  render () {
    ...
    return (
      <div className='panorama text-list'>
        <div ref='list' className='text-list--inner' style={innerStyle} onClick={this.onClickHandler}>
          {this.renderItems()}
        </div>
      </div>
    );
